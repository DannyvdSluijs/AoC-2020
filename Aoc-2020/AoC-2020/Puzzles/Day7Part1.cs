...
namespace Aoc_2020.Puzzles
{
    public static class Day7Part1
    {
        ...
        {
            var input = @"posh crimson bags contain 2 mirrored tan bags, 1 faded red bag, 1 striped gray bag.
...
dotted silver bags contain 1 muted blue bag, 3 shiny teal bags, 2 plaid coral bags, 1 dim aqua bag.".Split("\n");
            ...
            var buffer = input.Where(b => b.Contains("shiny gold") && !b.StartsWith("shiny gold"));

            while (buffer.Any())
            {
                var bag = buffer.Take(1).First().Split("bags contain").First();
                buffer = buffer.Where(b => !b.StartsWith(bag));
                match.Add(bag);

                buffer = buffer.Concat(input.Where(b => b.Contains(bag) && !b.StartsWith(bag)));
            }

            Console.WriteLine($"Found {match.Distinct().Count()} options for your bag"); // 172
        }
    }
}