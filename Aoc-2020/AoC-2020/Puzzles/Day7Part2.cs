...
namespace Aoc_2020.Puzzles
{
    ...
    {
        static readonly string[] input = @"posh crimson bags contain 2 mirrored tan bags, 1 faded red bag, 1 striped gray bag.
...
dotted silver bags contain 1 muted blue bag, 3 shiny teal bags, 2 plaid coral bags, 1 dim aqua bag.".Split("\n");
        
        public static void Execute()
        {
            Console.WriteLine($"The Shiny Gold bag must contain {GetBagSize("shiny gold") - 1} bags."); // 39645
        }

        private static long GetBagSize(string bagName)
        {
            var bag = input.First(bags => bags.StartsWith(bagName));

            if (bag.Contains("contain no other bags."))
            {
                return 1;
            }

            return bag.Split(" contain ")
                .Last()
                .Replace(".", "")
                .Split(",")
                .Select(i => int.Parse(i.Trim().Substring(0, 1)) * GetBagSize(i.Trim().Substring(2)))
                .Sum() + 1;
        }
    }
}