...
{
    public static class Day21Part1
    {
...
".Trim().Split("\n");
            ...
            var uniqueAllergens = input.Select(line => line.Split("contains").Last().Trim().Trim(')').Split(", "))
                .SelectMany(x => x)
                .Distinct();
            var ingredients = input.Select(line => line.Split(" (").First().Split(" "))
                .SelectMany(x => x);

            var results = new Dictionary<string, IEnumerable<string>>();
            foreach (var allergen in uniqueAllergens)
            {
                var lines = input.Where(line => line.Contains(" " + allergen))
                    .Select(line => line.Split(" (").First().Split(" "))
                    .ToArray();

                IEnumerable<string> start = lines.First();
                var intersection = lines.Aggregate(start, (current, line) => current.Intersect(line));

                results.Add(allergen, intersection);
            }

            var toBeResolved = results.Where(i => i.Value.Count() > 1)
                .OrderBy(x => x.Value.Count())
                .ToList();
            var resolved = results.Where(i => i.Value.Count() == 1)
                .Select(i => i.Value.First())
                .ToList();

            while (true)
            {
                for (int i = 0; i < toBeResolved.Count; i++)
                {
                    var resolving = toBeResolved[i];
                    var value = resolving.Value.Where(x => !resolved.Contains(x))
                        .ToArray();
                    results.Remove(resolving.Key);
                    results.Add(resolving.Key, value);

                    if (value.Count() != 1) continue;

                    toBeResolved.Remove(resolving);
                    resolved.Add(value.First());
                }

                if (results.All(i => i.Value.Count() == 1))
                {
                    break;
                }
            }

            var ingredientsWithAllergens = results.Select(x => x.Value.First());
            var ingredientsWithoutAllergens = ingredients.Where(x => !ingredientsWithAllergens.Contains(x));

            Console.WriteLine($"The answer is {ingredientsWithoutAllergens.Count()}"); //2282
        }
    }
}