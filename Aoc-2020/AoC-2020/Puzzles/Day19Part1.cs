...
{
    public static class Day19Part1
    {
...
"
            ...
            var r = ResolveRule(rules[0]).Select(i => i.Replace(" ", "")).ToArray();
            Console.WriteLine($"Resolved rules for 0; Resulted in {r.Count()} possibilities.");

            var msgLines = input.Last().Split("\n");
            var matching = r.Intersect(msgLines);

            Console.WriteLine($"The answer is {matching.Count()}"); // 109
        }
