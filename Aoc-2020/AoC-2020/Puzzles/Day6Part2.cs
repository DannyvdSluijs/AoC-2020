...
oc";
            ...
            var groupAnswers = groups.Select(i => i.Split("\n").Select(row => row.ToCharArray()));
            var answers = groupAnswers .Select(g =>
                {
                    var result = g.First();
                    foreach (var charse in g)
                    {
                        result = result.Intersect(charse).ToArray();
                    }
            ...
            Console.WriteLine($"The sum off all unique answers is {answers.Sum()}"); // 3232
        }
    }
}