...
5 + 3 * 2 * (9 + 6 * 6 + 9) * 3
";
            var answer = input.Trim().Split("\n").Select(Resolve).Select(Convert.ToInt64).Sum();

            Console.WriteLine($"The answer is {answer}"); // 70722650566361
        }

        private static long Resolve(string input)
        {
            while (input.Contains("("))
            {
                var offset = input.IndexOf("(", StringComparison.Ordinal);
                var sub = trimUntilMatchingClosingBracket(input.Substring(offset));
                var value = Resolve(sub);
                input = input.Replace($"({sub})", value.ToString());
            }

            while (input.Contains("+"))
            {
                var pieces = input.Split(" ").ToList();
                var offset = pieces.IndexOf("+");
                var value = long.Parse(pieces[offset - 1]) + long.Parse(pieces[offset + 1]);
                input = input.Replace($"{pieces[offset - 1]} + {pieces[offset + 1]}", value.ToString());
            }
        ...
        public static string trimUntilMatchingClosingBracket(string input)
        {
            var count = 1;
