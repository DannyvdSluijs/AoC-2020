...
LLLLLLL.LLLLLLLL.LLLLLLL.LLLLLLLLLLLLLL.LLLLLLL.LLLLLL.LL.LLLLLLL.LLLLLLLL.LLLLLLLLLLLLLLL".Split("\n").Select(row => row.ToCharArray()).ToArray();
            ...
            while (true)
            {
                ...
                if (MapToString(map) == MapToString(flippedMap))
                {
                    ...
                    Console.WriteLine($"In the end there are {occupiedSeats} seats occupied"); // 2265
                    return;
                }
        ...
        private static char[][] FlipMap(char[][] map)
        {
            ...
            for (int row = 0; row < numberOfRows; row++)
            {
                for (int column = 0; column < numberOfColumns; column++)
                {
                    ...
                    {
                        case 'L':
                            if (NumberOfAdjacentOccupiedSeats(map, row, column) == 0)
                            {
                                flippedMap[row][column] = '#';
                            }
                            break;
                        case '#':
                            if (NumberOfAdjacentOccupiedSeats(map, row, column) >= 4)
                            {
        ...
        private static int NumberOfAdjacentOccupiedSeats(char[][] map, int row, int column)
        {
            var numberOfOccupiedSeats = 0;
            var occupied = '#';
            if (GetCharFromMap(map, row - 1, column - 1) == occupied)
            {
                numberOfOccupiedSeats++;
            }
            if (GetCharFromMap(map, row - 1, column) == occupied)
            {
                numberOfOccupiedSeats++;
            }
            if (GetCharFromMap(map, row - 1, column + 1) == occupied)
            ...
            if (GetCharFromMap(map, row, column - 1) == occupied)
            {
                numberOfOccupiedSeats++;
            }
            if (GetCharFromMap(map, row, column + 1) == occupied)
        ...
        private static char GetCharFromMap(char[][] map, int row, int column)
        {
