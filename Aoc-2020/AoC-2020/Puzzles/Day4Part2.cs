...
hgt:183cm cid:187 byr:2019 ecl:xry iyr:2013 pid:164cm hcl:#18171d eyr:2021";
            ...
            var validPassports = input.Split("\n\n")
                ...
                .Where(p => int.TryParse(p["byr"], out var byr) && byr >= 1920 && byr <= 2002)
                .Where(p => int.TryParse(p["iyr"], out var iyr) && iyr >= 2010 && iyr <= 2020)
                .Where(p => int.TryParse(p["eyr"], out var eyr) && eyr >= 2020 && eyr <= 2030)
                .Where(p => ValidHeight(p["hgt"]))
                .Where(p => ValidHairColor(p["hcl"]))
                .Where(p => ValidEyeColor(p["ecl"]))
                .Where(p => ValidPid(p["pid"]));

            Console.WriteLine($"Found {validPassports.Count()} valid passports"); // 127
        }

        private static bool ValidHeight(string input)
        {
            var type = input.Substring(input.Length - 2);
            if (type != "cm" && type != "in")
            {
                return false;
            }
            var size = Int32.Parse(input.Substring(0, input.Length - 2));
        ...
        {
            return input.StartsWith('#') && IsHex(input.Substring(1));
        }

        private static bool ValidEyeColor(string input)
        {
            string[] valid = {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"};
            return valid.Contains(input);
        }

        private static bool ValidPid(string str)
        {
            if (str.Length != 9)
            {
                return false;
